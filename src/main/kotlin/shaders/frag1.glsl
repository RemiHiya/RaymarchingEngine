...
float sphereSDF(SDFObject sphere, vec3 p) {
    float dist = length(p - sphere.position) - sphere.scale;
    return dist;
}
float cubeSDF(SDFObject cube, vec3 p) {
    vec3 d = abs(p - cube.position) - cube.scale;
...
float map(vec3 ro) {
    if(objects.length() > 0) {
        float m;
        if (objects[0].type == 0) {
            m = sphereSDF(objects[0], ro);
        } else if (objects[0].type == 1) {
            m = cubeSDF(objects[0], ro);
        }
        for(int i=1; i<objects.length(); i++) {
            float d;
            if (objects[i].type == 0) {
                d = sphereSDF(objects[i], ro);
            } else if (objects[i].type == 1) {
                d = cubeSDF(objects[i], ro);
            } else if (objects[i].type == 3) {
                d = sdPlane(vec3(0, 0, -5), vec4(0, 0, 0, 50));
            }
...
void main() {
    objects[0] = SDFObject(0, vec3(0, 1+2*sin(u_time), -5), 1);
    objects[1] = SDFObject(1, vec3(0, -5, -5), 4);
    vec3 col;
